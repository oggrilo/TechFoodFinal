...
def PrecoLanche():
    sql = "select * from lanche where nomeLanche = '{Bauru}'"
    con.execute(sql)
...
def Preco ():
    print('O preço do Bauru é: R$' + str(PrecoLanche()) + ',00.')

def QuantidadeLanche():
    sql = "select * from lanche where quantidadeLanche = '{Bauru}'"
    con.execute(sql)
...
def Selecao (nomeLanche, quantidadeLanche):
    ...
    while (this.quantidade < 0) or (this.quantidade > QuantidadeLanche()):
        if (this.quantidade < 0) or (this.quantidade > QuantidadeLanche()):
            ...
            this.quantidade = int(input())
    sql = "update lanche set quantidadeLanche = '{}' where nomeLanche = '{Bauru}'".format(nomeLanche,quantidadeLanche)
    con.execute(sql)
